...
Codex CLI 子进程优先沿用已有代理环境变量；未显式配置时，Windows 上自动读取已启用的系统手动代理，并保持本机地址直连。此操作不修改系统或其他进程的代理配置。CLI 配置生成默认最多等待 180 秒，可通过 `THERMAL_CODEX_TIMEOUT_S` 设置为 15–600 秒；页面显示等待时长，并分别提示启动失败、连接超时和配置校验失败。系统仅配置 PAC 脚本时，请另行提供 CLI 可用的代理环境变量。

Codex 可从空配置新增热源。顶部、底部、左右、前后及全部外表面采用本地几何选区映射；轴向选区取对应方向最外侧 3% 范围内、法向朝向该方向的外表面三角面。草案列出热源功率、起止时间和面数，确认前可通过“应用后修改”查看或调整。缺少热源、选区为空、面编号无效或模型不匹配时，草案不能确认运行。
