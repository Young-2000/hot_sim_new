...
仿真助手提供两种模式：`local` 使用本地规则解析器，无需联网；`codex` 通过 OpenAI Responses API 生成配置，需先设置 `OPENAI_API_KEY`，可用 `OPENAI_MODEL` 指定模型（默认 `gpt-5.2`）。Codex 模式请求失败或未配置密钥时会明确提示，不会静默切换到本地模式。
